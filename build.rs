...
mod tonic {
    ...
    /// This function is responsible for building the Massa protobuf abi and
    ...
    pub fn build() -> Result<(), Box<dyn std::error::Error>> {
        ...
        generate_doc(&protos)
            .map_err(|e| format!("protobuf documentation error: {:?}", e))?;
    ...
    fn find_protos(
        ...
        let glob_pattern =
            format!("{dir_path}/**/*.proto", dir_path = dir_path);
        let paths = glob(&glob_pattern)?.flatten().collect();

        Ok(paths)
    }

    /// Generate markdown and HTML documentation for the given protocol buffer
    /// files
    fn generate_doc(
        protos: &Vec<PathBuf>,
